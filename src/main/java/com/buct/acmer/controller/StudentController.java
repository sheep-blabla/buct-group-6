...
public class StudentController {
    ...
    public PublicProperty<Page<Student>> selectsome(@PathVariable("currentPage") Integer currentPage,
    ...
    public PublicProperty<Page<Student>> selectAll(@PathVariable("currentPage") Integer currentPage,
        ...
        Page<Student> page = studentService.selectStudent(currentPage, pageSize);
        return new PublicProperty(200,"success", page);
    }

    @ApiOperation("按学号姓名查询学生信息")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "currentPage",value = "当前页数",dataType = "int",required = true),
            @ApiImplicitParam(name = "pageSize",value = "页面大小",dataType = "int",required = true),
            @ApiImplicitParam(name = "stuNo",value = "学号",dataType = "String",required = true),
            @ApiImplicitParam(name = "stuName",value = "姓名",dataType = "String",required = true)
    })
    @GetMapping("/all/select/{currentPage}/{pageSize}/{stuNo}/{stuName}")
    public PublicProperty<Page<Student>> searchByField(@PathVariable("currentPage") Integer currentPage,
        ...
        Page<Student> page=studentService.selectByVal(currentPage, pageSize, cfId, stuName);
        return new PublicProperty<>(200, "success", page);
    }
